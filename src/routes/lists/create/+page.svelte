...
<div class="container">
	<header class="header">
		<div class="header-title-container">
			<h1 class="header-title">Criar Nova Lista</h1>
			<p class="header-subtitle">Defina o nome da sua lista de compras</p>
		</div>
	</header>
		...
		<form onsubmit={handleSave}>
			<Grid direction="column" gap="m">
				<wa-input
					...
					label="Nome da Lista *"
					type="text"
					placeholder="Ex: Supermercado, Casa, Lidl..."
					value={listName}
				...
				<div class="sharing-section">
					<Grid align="end" gap="s" class="email-input-grid">
						<wa-input
							id="share-email"
							...
							class="input-grow"
						></wa-input>
						<wa-button variant="neutral" onclick={addEmail} class="btn-add-email">
				...
				<Grid gap="m" fullWidth class="button-grid">
					...
					<wa-button variant="neutral" href="/">
						Cancelar
					</wa-button>
				</Grid>
			</Grid>
...
<style>
	...
	.sharing-section {
		...
		border-top: 1px solid var(--wa-color-neutral-30);
		padding-top: var(--wa-space-l);
	}
	:global(.email-input-grid) {
		width: 100%;
	}
	:global(.input-grow) {
		flex-grow: 1;
